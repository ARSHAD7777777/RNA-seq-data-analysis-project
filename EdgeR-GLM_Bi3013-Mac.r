...
# Windows: to run the R-script write in the R Console window: source ("EdgeR-GLM_Bi3013-Mac.r")                                 #

# to install package edgeR
...
setwd("~/Desktop/tempBI3013")   # working directory
...
ROT <- ("~/Desktop/tempBI3013");
setwd (ROT);

IMG <- ("~/Desktop/tempBI3013/IMG");                 # folder for images

# folders for counttab and results
CountTAB <- ("~/Desktop/tempBI3013/CountTAB");       # Count table is placed here 
Results <- ("~/Desktop/tempBI3013/Results");         # output of DE genes result tables
...
x <- read.delim("CountTable.txt", row.names="Phatr3_ID");      # the Count tab is loaded into x
...
dev.print(png, file = "logFC avg-logCPM.png", width = 1024, height = 768)       #Works for mac
dev.off()
...
dev.print(png, file = "BCVplot.png", width = 1024, height = 768) #works for mac
dev.off()
